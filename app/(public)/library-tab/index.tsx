...
/**
 * The Library tab itself, not a redirect into another stack.
 *
 * This screen used to live under `(authed)` while the tab was a stub that
 * bounced to it on focus. The bounce left the tab bar behind and pushed a
 * Stack screen over the top, so Library was the only tab that looked like a
 * different app and could be navigated away from into nowhere. Every other tab
 * renders in place under `(public)`; this one does now too.
 *
 * Signed-out visitors are handled inside rather than by a redirect: the tab is
 * reachable, and each panel says what signing in would add.
 */
